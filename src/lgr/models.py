...
class Barcode(BarcodeHistoryMixin, models.Model):
    code = models.CharField(max_length=64, primary_key=True, unique=True, editable=True)
    ...
    def api_history(self):
        ...
        history = [
            {"user": h.person.nickname, "field": h.field, "old": h.old, "new": h.new,}
            for h in history
        ]
        return history
...
class History(models.Model):
    ...
    person = models.ForeignKey(Person, on_delete=models.CASCADE, related_name="changes")
...
class Loan(models.Model):
    ...
    person = models.ForeignKey(Person, related_name="loans", on_delete=models.PROTECT)
