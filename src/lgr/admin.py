...
class BarcodeAdmin(admin.ModelAdmin):
    ...
    def short_description(self, obj: models.Loan):
        ...
        description = format_html(
            '<span title="{}">{}</span>', obj.description, description
        )
        return description
    ...
    def quickadd_view(self, request, obj_id=None, *args, **kwargs):
        ...
        if request.method != "POST":
            ...
            if form.is_valid():
                BarcodeAdmin.process_quickadd(request, form.cleaned_data["barcodes"])
        ...
        return TemplateResponse(
            request, "admin/custom_change_form.html", context=context
        )
    ...
    def moveinto_vew(self, request, obj_id, *args, **kwargs):
        ...
        if request.method != "POST":
            ...
            if form.is_valid():
                BarcodeAdmin.process_moveinto(
                    request, form.cleaned_data["barcodes"], obj
                )
        ...
        if form.is_valid():
            ...
            for barcode in queryset.all():
                if not parent in barcode.all_children:
                    ...
                    messages.info(
                        request, "%s has now as parent %s" % (barcode, barcode.parent)
                    )
                else:
                    messages.error(
                        request,
    ...
    def changelist_view(self, request, extra_context={}):
        topbuttons = [
            {"url": reverse("admin:lgr_barcode_quickadd"), "title": "Quickadd"}
        ]
        extra_context.update({"topbuttons": topbuttons})
    ...
    def merge(self, request, queryset):
        ...
        for old in items:
            ...
            for count, barcode in enumerate(barcodes, 1):
                barcode.item = new
                barcode.save(user=user)
            messages.info(request, "Moved %s items from %s to %s" % (count, old, new))
            old.delete()
...
class LoanAdmin(admin.ModelAdmin):
    ...
    list_display = ("__str__", "taken_date", "returned_date", "item_count", "buttons")
    ...
    def process_return(
        self, request, loan: models.Loan, barcodes: List[models.Barcode]
    ):
