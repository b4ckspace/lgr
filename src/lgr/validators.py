...
BarcodeTuple = namedtuple(
    "BarcodeTuple", ["code", "item", "parent", "owner", "description"]
)
    ...
    for linenr, line in enumerate(value.splitlines(), 1):
        ...
        if match["barcode"]:
            ...
            # a range of barcodes
                ...
                code_range = (
                    "%s%s" % (prefix, str(i).zfill(max_len))
                    for i in range(start, end + 1)
                )
        else:
            raise ValidationError(
                "Line does not contain Owner or Barcode " f"(line: {linenr})"
            )
        ...
        if len(levelmap) - match["level"] < 1:
            raise ValidationError(
                "You can only ident one step (2 chars) at a time " f"(line: {linenr})"
            )
        ...
        for code in code_range:
            ...
            if match["item"]:
                ...
                if barcode_db:
                    raise ValidationError(
                        "Barcode is already known, remove " f"item (line: {linenr})"
                    )
                ...
                if barcode_db is None:
                    raise ValidationError(
                        f"Barcode {code} not found and no item given. "
                        f"(line: {linenr})"
                    )
