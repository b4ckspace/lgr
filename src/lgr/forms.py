...
class BarcodeMoveintoForm(forms.Form):
    barcodes = forms.CharField(
        widget=forms.Textarea, help_text=format_html("Enter one barcode per line.")
    )

    def clean_barcodes(self):
        text = self.cleaned_data["barcodes"]
        as_barcodes = [
            models.Barcode.objects.filter(pk=b).first() for b in text.splitlines()
        ]
        return as_barcodes
    ...
    form.fields.update(
        {
            "action": forms.CharField(
                widget=forms.HiddenInput, initial="move_multiple"
            ),
